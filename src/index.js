onPageReady(function () {
	if (window.alt1) {
		...
		// Begin Cache S. Issue-9 12/05/2023 -- Add ability to repeat the speaker's name for each dialogue box
		document.getElementById("repeat-speaker").addEventListener("change", function(evt) {
			repeatSpeaker = !evt.target.checked;
		});
		// End Cache S. Issue-9
...
// Cache S. Issue-9 - 12/05/2023 - Add ability to repeat the speaker's name for each dialogue box
var repeatSpeaker;
...
function stringify(dialogue, indentLevel=1, parent) {
	...
	if (dialogue == null) return "{{Transcript missing}}";
	...
	//The dialogue doesn't have a speaker.
		...
		if (!repeatSpeaker
			...
			// Cache S. Issue-9 12/05/2023 -- break up code to match other code blocks, and Add ability to repeat the speaker's name for each dialogue box
			retVal = "\n";
			retVal += "*".repeat(indentLevel);
			retVal += " ";
			retVal += "{{Tbox|";
